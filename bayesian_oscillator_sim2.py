...
for file in os.listdir(param_path):
    ...
    print(split_str)
#%%
if len(glob.glob(param_path + 'Sim_param_synch.npy')) == 0:
    # setting
    ...
    for st in range(State):
        ...
        if st == 0:
            ...
            K2_tr[:,:,st] = 0.77 * (np.ones((Nosc, Nosc)) - np.eye(Nosc))
        elif st == 1:
    ...
    save_name       = 'Sim_param_synch'
    fullpath_save   = param_path + save_name 
    np.save(fullpath_save, param_dict)
    #%%
elif len(glob.glob(param_path + 'Sim_param_synch.npy')) != 0:
