# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 17:44:26 2020
...
param_path = current_path + '\\save_data\\param_sim1\\'
...
for file in os.listdir(param_path):
    ...
    print(split_str)
#%%
if len(glob.glob(param_path + '*.npy')) == 0:
    # setting
    ...
    for st in range(State):
        ...
        if st == 0:
            K1_tr[:,:,st] = K1_tr[:,:,st] * np.tri(Nosc, Nosc).T
            K2_tr[:,:,st] = K2_tr[:,:,st] * np.tri(Nosc, Nosc).T
            
            K1_tr[0,1,st] = 0
            K2_tr[0,1,st] = 0
        elif st == 1:
            K1_tr[:,:,st] = K1_tr[:,:,st] * np.tri(Nosc, Nosc)
            K2_tr[:,:,st] = K2_tr[:,:,st] * np.tri(Nosc, Nosc).T
        elif st == 2:
            K1_tr[:,:,st] = K1_tr[:,:,st-1]
            K2_tr[:,:,st] = K2_tr[:,:,st-1]
    ...
    omega      = 2 * np.pi * np.array([3.95, 4.2, 4.62])#.uniform(2, 3, Nosc)
