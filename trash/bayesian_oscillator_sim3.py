...
for state in range(State):
    ...
    vis_heatmap(K_tr[:,:,state], vmin, vmax, ax, np.array(['Segment %d\n'%(state+1), 'osci. $j$', 'osci. $i$']), cbar_info, linewidths = 0.001, fontsize=28)
    fig.add_subplot(ax)
    if state == 0:
    ...
    vis_heatmap(Kest_ave[:,:,state], vmin, vmax, ax, np.array(['', 'osci. $j$', 'osci. $i$']), cbar_info, linewidths = 0.001)
    fig.add_subplot(ax)

ax = plt.Subplot(fig, outer[1])
ax.plot(Time[np.isnan(Changes)==False], Changes[np.isnan(Changes)==False]);
ax.scatter(Time[idx], Changes[idx], marker = 'o', c = 'red', label = 'Change point\n(KL div > $\\mu$ + 3SD)');
...
for i in range(State):
    ...
    vis_heatmap(K1_tr[:,:,i], vmin, vmax, a_ax, np.array(['Segment %d\n $a_{ij}$'%(i+1), 'osci. $j$', 'osci. $i$']), cbar_info, linewidths = 0.001, fontsize=28)
